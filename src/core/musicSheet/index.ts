...
class MusicSheetClazz implements IInjectable {
    ...
    /**
     * 换源：用新来源的歌曲替换歌单中指定位置的旧歌曲，保留原有位置与排序
     * @param sheetId - 歌单 id
     * @param oldMusicItem - 歌单中的原歌曲
     * @param newMusicItem - 从其它插件找到的新来源歌曲
     * @returns 是否替换成功
     */
    async replaceMusic(
        sheetId: string,
        ...
        const musicList = this.getSortedMusicListBySheetId(sheetId);
        const index = musicList.musicList.findIndex(_ =>
            isSameMediaItem(_, oldMusicItem),
        );
        if (index === -1) {
            return false;
        }
        const newArray = musicList.musicList.slice();
        // 保留原位置的时间戳与排序索引，避免影响歌单顺序
        newArray[index] = {
            ...newMusicItem,
            $timestamp: oldMusicItem.$timestamp,
            $sortIndex: oldMusicItem.$sortIndex,
        };
        musicList.manualSort(newArray);
        await storage.setMusicList(sheetId, musicList.musicList);
        ee.emit("UpdateMusicList", {
            sheetId,
            updateType: "length",
