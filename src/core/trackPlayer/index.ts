...
/** 当前播放列表的自动下载策略（true=歌单播放列表，false=搜索等临时列表） */
let playListAutoDownload = true;
...
class TrackPlayer extends EventEmitter<{
    ...
    async remove(musicItem: IMusic.IMusicItem): Promise<void> {
        ...
        if (shouldPlayCurrent === true) {
            await this.play(currentMusic, true, playListAutoDownload);
    ...
    async play(
        ...
        try {
            // 如果不传参，默认是播放当前音乐
            const isResumePlay = musicItem == null;
            if (!musicItem) {
            ...
            // 自动下载开关：
            // - 显式传入（如搜索页传 false）优先；
            // - 恢复播放 / 播放列表内切歌（含队列点播）沿用当前播放列表的策略；
            // - 其余（歌单、推荐等列表点歌）默认自动下载。
            const _autoDownload =
                autoDownload ??
                (isResumePlay || this.isInPlayList(musicItem)
            ...
            // 1.1 本地没有已下载文件且允许自动下载时，下载到下载目录（不阻塞在线播放）
            if (
                _autoDownload &&
                !localPath &&
                !LocalMusicSheet.isLocalMusic(musicItem)
            ...
            if (info && this.isCurrentMusic(musicItem)) {
                // 关键修复：更新通知栏/锁屏 metadata 前必须 sanitize artwork，
                // 否则 artwork 为空字符串 "" 会被原生 Uri.parse("") 抛
                // IllegalArgumentException 导致整个 app 闪退（播放失败即崩溃）。
                const mergedTrack = this.patchMediaArtwork(
                    this.mergeTrackSource(track, info) as Track,
                );
            ...
            if (
                ...
                this.play(musicItem, forcePlay, autoDownload);
    ...
    async playWithReplacePlayList(
        ...
        if (newPlayList.length !== 0) {
            ...
            await this.play(musicItem, true, autoDownload);
        }
    }
